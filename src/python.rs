...
fn extract_scenario(d: &Bound<'_, PyDict>) -> PyResult<Scenario> {
    ...
    Ok(Scenario {
        ...
        instance_shuffle: opt_bool("instance_shuffle")?.unwrap_or(true),
        instance_shuffle_seed: opt_u64("instance_shuffle_seed")?.unwrap_or(0),
        future_telling: opt_bool("future_telling")?.unwrap_or(false),
        future_telling_checkpoint: opt_f64("future_telling_checkpoint")?.unwrap_or(1.0),
        future_telling_cores: opt_usize("future_telling_cores")?,
        future_telling_tolerance: opt_f64("future_telling_tolerance")?.unwrap_or(0.0),
